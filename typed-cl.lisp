(defpackage :typed-cl
  (:nicknames "TYPED")
  (:use :cl)
  (:export "DEF" "LDEF" "LIST-OF" "VARS" "VARS*"))
(in-package :typed-cl)

(deftype any () t)

(defun make-keyword (symbol)
  "make a keyword out of a symbol"
  (read-from-string (concatenate 'string ":" (symbol-name symbol))))

(defun is-type (s)
  "a future function that seems not to be possble to implement at the moment"
  (declare (ignore s))
  t)
; my initial attempt at is-type
...
(defmacro def (rettype fname args &body body)
  ...
  &rest parameters are not type checked in SBCL (see sbcl-bug.txt)
  "
